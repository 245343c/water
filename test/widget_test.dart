...
void main() {
  Future<AppUser> signInMockCustomer(
    AuthRepository auth,
    WaterPlantRepository repo,
    String phone, {
    String name = 'Customer',
  }) async {
    expect(await auth.requestCustomerOtp(phone), isNull);
    final error = await auth.verifyCustomerOtp(phone: phone, otp: '123456');
    expect(error, isNull);
    final user = auth.currentUser!;
    repo.linkContractCustomerOnLogin(userId: user.id, phone: user.phone);
    auth.markCustomerOnboardingComplete(user.id, name: name);
    return user;
  }
  ...
  group(
    'paused customer UI flow',
    skip: 'Customer UI is paused for the admin + driver release.',
    () {
      testWidgets('Customer home renders linked mock shop', (tester) async {
        final auth = AuthRepository(
          restoreFirebaseSession: false,
          useMockCustomerOtp: true,
        );
        final repo = WaterPlantRepository();
        await signInMockCustomer(
          auth,
          repo,
          '9876543210',
          name: 'Lakshmi Devi',
      ...
      testWidgets('Customer shell renders home tab body', (tester) async {
        ...
        expect(
          find.text('You can order from your linked plant'),
          findsOneWidget,
        );
      ...
      testWidgets('Customer home renders multiple linked monthly plants', (
        ...
        final repo = WaterPlantRepository();
        await signInMockCustomer(auth, repo, '9632580741', name: 'Abi');
        ...
        expect(
          find.text('You can order from your linked plants'),
          findsOneWidget,
        );
        expect(find.text('Sri Sai RO Water Plant'), findsOneWidget);
      ...
      testWidgets('Customer shop detail sends a water request', (tester) async {
        ...
        final user = await signInMockCustomer(
          auth,
          repo,
          '9876543210',
          name: 'Lakshmi Devi',
      ...
      testWidgets('Monthly customer orders tab shows request status cards', (
        ...
        final beforeStats = repo.monthlyStatsForCustomer(
          linkedCustomerId,
          month,
        );
        ...
        final afterStats = repo.monthlyStatsForCustomer(
          linkedCustomerId,
          month,
        );
      ...
      test(
        'Multi-plant customer request stays with selected plant account',
        () async {
          final auth = AuthRepository(
            restoreFirebaseSession: false,
            useMockCustomerOtp: true,
      ...
      test(
        ...
        () async {
          ...
          final repo = WaterPlantRepository();
          final user = await signInMockCustomer(auth, repo, '9876543210');
      ...
      test(
        'Driver accepted and started states are tracked for customer order',
        () async {
          final auth = AuthRepository(
            restoreFirebaseSession: false,
            useMockCustomerOtp: true,
